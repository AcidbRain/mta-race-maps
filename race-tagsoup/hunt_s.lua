...
local defmodspd = 16 -----------
local defaitime = 4  -----------
local defsplimit = 250  --------
local defstartdely = 3
local defpause = 6
local modspd = ""
local aitime = ""
local splimit = ""
...
addEventHandler("onRaceStateChanging", root,function ( state )
	...
	if (state == "Running") and (activestate == 1) then
		if startdely == 0 then
			...
			--triggerClientEvent(root,"onclientfirstplayerDEAD",core)
			showtext()
			delytimer = setTimer(function()
				--triggerClientEvent(root,"onclientpausestop",core)
				hidetext()
...
function activate()
	...
	botai(firstplayer)
	triggerClientEvent(root,"onnewfirstplayer",firstplayer,firstplayer)
end
function botai(benfirst)
	...
	if (racestate == "Running") or (racestate == "MidMapVote") then
		...
		if firstplayer ~= benfirst then
			...
			triggerClientEvent(root,"onnewfirstplayer",firstplayer,benfirst)
		end
		...
		if getElementData(benfirst, "state") == "dead" then
			...
			triggerClientEvent(root,"onclientfirstplayerDEAD",firstplayer)
			pausetimer = setTimer(function()
				destroyElement(pause_MissionTimer)
				hidetext()
				botai(firstplayer)
				if getElementData(benfirst, "state") ~= "dead" then
					triggerClientEvent(root,"onclientpausestop",firstplayer)
				end
			end, (pause*1000), 1)
		else
...
addEventHandler("onNewPlayerDetected", root,function ()
	local pla = source
